...
# 请求openai访问的图片生成接口
def openai_image_query(
    base_url, api_key, model, query, n=1, response_format="b64_json"
):
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}

    data = {"model": model, "prompt": query, "n": n, "response_format": response_format}
...
def common_image(cfg, name, prompt, is_dify):
    ...
    if api_type == "image":
        result = openai_image_query(
            base_url, api_key, model, prompt, image_count, response_format
        )
...
def save_base64_image(image_data, prefix="openai_image"):
    ...
    image_path = (
        image_dir
        / f"{prefix}_{int(time.time() * 1000)}_{random.randint(1000, 9999)}.png"
    )
    image_path.write_bytes(base64.b64decode(image_data))
    return image_path


def build_edited_image_path(target_dir, source_image_path, suffix=".png"):
    target_path = Path(target_dir)
    target_path.mkdir(parents=True, exist_ok=True)

    source_path = Path(str(source_image_path or "").strip())
    stem = source_path.stem or "image"
    file_suffix = suffix or source_path.suffix or ".png"
    return target_path / f"{stem}_edits_{int(time.time() * 1000)}{file_suffix}"


def save_binary_image_to_path(image_bytes, image_path):
...
def build_image_component(image_ref, prefix="openai_image", save_path=None):
    ...
    if image_ref.startswith("http://") or image_ref.startswith("https://"):
        if save_path:
            try:
                ...
                image_path = save_binary_image_to_path(response.content, save_path)
    ...
    if image_ref.startswith("data:"):
        try:
            ...
            if save_path:
                image_path = save_binary_image_to_path(
                    base64.b64decode(encoded), save_path
                )
            else:
    ...
    try:
        ...
        if save_path:
            image_path = save_binary_image_to_path(image_bytes, save_path)
        else:
...
def decode_generation_result(result):
    ...
    for item in data_list:
        ...
        if b64_json:
            image_component, _ = build_image_component(
                f"base64://{b64_json}", prefix="openai_image"
            )
            if image_component:
                chain.append(image_component)
...
def openai_edit_image_query(
    base_url, api_key, model, image_path, prompt, size=None, response_format="b64_json"
):
    base_url = normalize_openai_base_url(base_url)
...
def edit_image_with_openai(cfg, image_path, prompt):
    ...
    size = cfg.get("size", "")
    response_format = cfg.get("response_format", "b64_json")
    editimage_dir = str(cfg.get("editimage_dir", "")).strip()
    ...
    default_suffix = source_path.suffix or ".png"
    for item in data_list:
        image_url = item.get("url")
        b64_json = item.get("b64_json")
        save_path = None
        if editimage_dir:
            save_path = build_edited_image_path(
                editimage_dir, image_path, suffix=default_suffix
            )

        if image_url:
            image_component, saved_path = build_image_component(
                image_url,
                prefix="openai_edit",
                save_path=save_path,
            )
...
def move_image(image_src, target_dir):
    ...
    def save_binary_image(image_bytes, suffix=".png"):
        image_path = (
            target_path
            / f"image_{int(time.time() * 1000)}_{random.randint(1000, 9999)}{suffix}"
        )
        image_path.write_bytes(image_bytes)
